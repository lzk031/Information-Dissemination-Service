...
func testPutGetDelete() {
	...
	// delete the key
	replyD, err := st.Delete("keyputget:1")
...
// Delete a key before its lease expires
// expect a revoke msg from storage server
func testDeleteBeforeLeaseExpire() {
	key := "revokekey:1"
...
// Delete a key after its lease expires
// expect no revoke msg received from storage server
func testDeleteAfterLeaseExpire() {
	key := "revokekey:2"
